...
说明：`gitee.com` 是国内代码仓库与说明页地址，不是静态网页应用托管域名。当前在线应用主入口为自有域名，备用入口为 Cloudflare Pages 域名。不同站点来源的本地学习数据不会自动互通；跨站点使用前应先在 `设置 -> 词卡与备份` 导出备份。
...
Cloudflare Pages: backup online app, built from the main branch
```
...
```text
软件代码：LICENSE
用户使用边界：TERMS_OF_USE.md
官方内容许可：CONTENT_LICENSE.md
贡献者内容规则：CONTRIBUTOR_CONTENT_POLICY.md
社区包下架规则：TAKEDOWN_POLICY.md
发布校验：docs/PACKAGE_INTEGRITY_AND_RELEASE_CHECKSUMS.md
来源台账模板：docs/CARD_PACKAGE_SOURCE_LEDGER_TEMPLATE.md
社区包提交模板：docs/COMMUNITY_CARD_PACKAGE_SUBMISSION_TEMPLATE.md
```
